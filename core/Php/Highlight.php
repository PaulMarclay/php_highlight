...
class Php_Highlight
{
    ...
    {
        // begin of content of require_once 'PHP/Compat/Constant/T.php';
        ...
        // en of content of require_once 'PHP/Compat/Constant/T.php';
     ...
     */
    function toHtml($return = false, $linenum = false, $format = null, $funcref = true, $baseLine = 0, $highlightTheLine = null)
    {
        // Ensure source has been loaded
        if ($this->_source == false) {
            return false;
        ...
        $source = array_splice($source, 1, count($source) - 2);
        $out = "<code>\n";
        foreach ($source as $i => $line) {
            $out .= '    ';
            $out .= (($highlightTheLine && $baseLine + $i == $highlightTheLine) ? '<div style="border-color: red;border-width: 1;border-style: solid;background-color: yellow;">' : '<div>');
            if ($linenum === true) {
                $out .= sprintf($format, $baseLine + $i);
            }
